...
void VideoPlayer::OnNativeVideoOpened(const std::uint32_t width,
                                      ...
                                      const VideoLayout::OutputRect contentRect,
                                      std::filesystem::path selectedVideo)
{
    {
        std::scoped_lock lock(wakeMutex_);
        const std::uint64_t nextSession =
            session_.fetch_add(1, std::memory_order_acq_rel) + 1;
        selectedVideo_ = VideoRequest{ std::move(selectedVideo), width, height,
                                       contentRect, nextSession };
...
std::optional<std::filesystem::path> VideoPlayer::PickDedicatedAudio()
{
    ...
    for (const auto& source : sources) {
        ...
        spdlog::info("Selected dedicated main-menu audio: {}",
                     Utf8Path(source));
        return source;
    }
    spdlog::warn(
...
VideoPlayer::PickDedicatedAudioForVideo(const std::filesystem::path& video,
                                        const bool allowRandomFallback)
{
    ...
    for (const auto& source : sources) {
        ...
        spdlog::info("Selected same-name main-menu audio for {}: {}",
                     Utf8Path(video.filename()), Utf8Path(previousAudio_));
    ...
    for (const auto& source : sources) {
        ...
        spdlog::info("Selected random main-menu audio for silent video {}: {}",
                     Utf8Path(video.filename()), Utf8Path(source));
        return source;
    }
    ...
    const std::filesystem::path& path) const
{
    ...
    for (unsigned int index = 0; index < format->nb_streams; ++index) {
        ...
        const bool usable = result >= 0 && decoder->sample_rate > 0 &&
                            decoder->ch_layout.nb_channels > 0;
...
void VideoPlayer::AdjustVolume(const float delta)
{
    ...
    while (!volume_.compare_exchange_weak(
        current, std::clamp(current + delta, 0.0F, 2.0F),
...
void VideoPlayer::Worker(std::stop_token stopToken)
{
    ...
    while (!stopToken.stop_requested()) {
        ...
        bool videoActive = false;
        {
            ...
            videoActive = nativeVideoActive_.load(std::memory_order_acquire);
            if (selectedVideo_) {
        ...
        if (!videoActive || selected.path.empty() ||
            selected.session != handledSession) {
            continue;
        }
        spdlog::info("Main-menu session {} selected video: {}", handledSession,
                     Utf8Path(selected.path));
        DecodeSession(selected, selected.session, stopToken);
        latestFrame_.store({}, std::memory_order_release);
                                     ...
                                     const bool overrideAudio)
{
    ...
    int result =
        avformat_open_input(&format, nativePath.c_str(), nullptr, nullptr);
    if (result < 0) {
        spdlog::error("Audio decoder could not open {}: {}", nativePath,
                      AvError(result));
        cleanUp();
        return false;
    }
    ...
    const auto openFirstDecodableAudioStream = [&] {
        for (unsigned int index = 0; index < format->nb_streams; ++index) {
            ...
            if (candidateResult >= 0 && candidateDecoder->sample_rate > 0 &&
                candidateDecoder->ch_layout.nb_channels > 0) {
    ...
    int layoutResult = 0;
    if (av_channel_layout_check(&decoder->ch_layout) &&
        decoder->ch_layout.order != AV_CHANNEL_ORDER_UNSPEC) {
        layoutResult =
            av_channel_layout_copy(&inputLayout, &decoder->ch_layout);
    } else {
        av_channel_layout_default(&inputLayout, inputChannels);
    }
    AVChannelLayout outputLayout = AV_CHANNEL_LAYOUT_STEREO;
    if (layoutResult >= 0) {
        layoutResult = swr_alloc_set_opts2(&resampler, &outputLayout,
                                           kOutputFormat, kOutputSampleRate,
                                           &inputLayout, decoder->sample_fmt,
                                           decoder->sample_rate, 0, nullptr);
    }
    av_channel_layout_uninit(&inputLayout);
    av_channel_layout_uninit(&outputLayout);
    if (layoutResult < 0 || !resampler || swr_init(resampler) < 0) {
    ...
    spdlog::info("Playing audio stream {} with {} decoder: {} Hz, {} channels",
                 audioStream, codec->name, decoder->sample_rate, inputChannels);
    ...
    while (!stopToken.stop_requested() && sessionActive() && !playbackFailed) {
        ...
        output.SetVolume(overrideAudio || OriginalAudioAudible() ? Volume()
                                                                 : 0.0F);
        result = av_read_frame(format, packet);
        if (result < 0) {
            const std::int64_t seekTarget =
                stream->start_time == AV_NOPTS_VALUE ? 0 : stream->start_time;
            if (av_seek_frame(format, audioStream, seekTarget,
        ...
        while (!playbackFailed &&
               avcodec_receive_frame(decoder, decoded) == 0) {
            if (stopToken.stop_requested() || !sessionActive()) {
                break;
            }
            if (!PlaybackGate::MayAdvance()) {
            ...
            const int outputCapacity = static_cast<int>(av_rescale_rnd(
                delayedSamples + decoded->nb_samples, kOutputSampleRate,
            ...
            const int converted = swr_convert(
                resampler, outputPlanes, outputCapacity,
...
bool VideoPlayer::DecodeSession(const VideoRequest& request,
                                const std::uint64_t session,
                                std::stop_token stopToken)
{
    ...
    AVPacket* packet = nullptr;
    AVFrame* decoded = nullptr;
    VideoScaling::FrameScaler converter;
    ...
    int result =
        avformat_open_input(&format, nativePath.c_str(), nullptr, nullptr);
    if (result < 0) {
        spdlog::error("FFmpeg could not open {}: {}", nativePath,
                      AvError(result));
        cleanUp();
        return false;
    }

    // MP4 and Matroska expose their track list during avformat_open_input, so
    // avoid an unnecessary packet-probing pass before opening the decoder.
    ...
    if (videoStream < 0 || !codec) {
        spdlog::error("FFmpeg found no decodable video stream in {}",
                      nativePath);
        cleanUp();
        return false;
    }

    spdlog::info("Opening {} decoder for video stream {}", codec->name,
                 videoStream);
    ...
    if (result < 0 || decoder->width <= 0 || decoder->height <= 0) {
        spdlog::error("FFmpeg could not initialize decoder {}: {}", codec->name,
                      AvError(result));
        cleanUp();
        return false;
    }
    ...
    spdlog::info("Opened {} decoder: {}x{}, {:.3f} FPS", codec->name,
                 decoder->width, decoder->height, framesPerSecond);

    const std::uint32_t outputWidth = request.outputWidth;
    const std::uint32_t outputHeight = request.outputHeight;
    const VideoLayout::OutputRect contentRect = request.contentRect;
    if (outputWidth == 0 || outputHeight == 0 || contentRect.width == 0 ||
        contentRect.height == 0 ||
        static_cast<std::uint64_t>(contentRect.x) + contentRect.width >
            outputWidth ||
        static_cast<std::uint64_t>(contentRect.y) + contentRect.height >
            outputHeight) {
    ...
    bool loggedScalingInfo = false;
    bool loggedScalingFallback = false;
    ...
    while (!stopToken.stop_requested() && SessionActive(session)) {
        ...
        if (result < 0) {
            const std::int64_t seekTarget =
                stream->start_time == AV_NOPTS_VALUE ? 0 : stream->start_time;
            if (av_seek_frame(format, videoStream, seekTarget,
                              AVSEEK_FLAG_BACKWARD) < 0) {
        ...
        while (avcodec_receive_frame(decoder, decoded) == 0) {
            ...
            if (!VideoScaling::ApplyCenterCrop(decoded, contentRect.width,
                                               contentRect.height)) {
            ...
            VideoScaling::EnsureColorMetadata(decoded);
            ...
            try {
                frame->pixels.resize(static_cast<std::size_t>(frame->rowPitch) *
                                     frame->height);
            } catch (const std::bad_alloc&) {
                spdlog::error(
            ...
            // The frame pool is rebuilt between sessions. Newly grown vector
            // storage initializes the untouched aspect padding to black.
            AVFrame outputFrame{};
            outputFrame.format = AV_PIX_FMT_BGRA;
            outputFrame.width = static_cast<int>(contentRect.width);
            outputFrame.height = static_cast<int>(contentRect.height);
            outputFrame.data[0] =
                frame->pixels.data() +
                static_cast<std::size_t>(contentRect.y) * frame->rowPitch +
                static_cast<std::size_t>(contentRect.x) * 4;
            outputFrame.linesize[0] = static_cast<int>(frame->rowPitch);
            outputFrame.color_primaries = decoded->color_primaries;
            outputFrame.color_trc = decoded->color_trc;
            outputFrame.colorspace = AVCOL_SPC_RGB;
            outputFrame.color_range = AVCOL_RANGE_JPEG;
            VideoScaling::EnsureColorMetadata(&outputFrame);
            if (!converter.Scale(decoded, &outputFrame,
            ...
            if (converter.FellBackToBicubic() && !loggedScalingFallback) {
                spdlog::warn(
                    "Spline36 was unavailable for this video; falling back "
                    "to Bicubic: {}",
                    converter.LastError());
                loggedScalingFallback = true;
            }
            if (!loggedScalingInfo) {
                const auto pixelFormat =
                    static_cast<AVPixelFormat>(decoded->format);
                spdlog::info(
                    "Preparing decoded frame: {}x{}, pixel format {}; {} "
                    "scaling to content rect ({}, {}) {}x{} on {}x{} carrier",
                    decodedWidth, decodedHeight,
                    av_get_pix_fmt_name(pixelFormat)
                        ? av_get_pix_fmt_name(pixelFormat)
                        : "unknown",
                    converter.UsedDirectCopy()
                        ? std::string_view("direct-copy")
                        : ScalingAlgorithmName(converter.ActiveAlgorithm()),
                    contentRect.x, contentRect.y, contentRect.width,
                    contentRect.height, outputWidth, outputHeight);
                loggedScalingInfo = true;
            }
